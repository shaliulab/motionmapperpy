...
def egoh5(h5, bindcenter=8, align=True, b1=2, b2=11, silent=False):
    ...
    if not silent:
        for t in tqdm(range(egoh5.shape[0])):
            rot_mat = np.array(
                [[dir_arr[t, 0], dir_arr[t, 1]], [-dir_arr[t, 1], dir_arr[t, 0]]]
            )
...
def plotLaggedEigenvalues(transitions, lags=None, numModes=5):
    ...
    for i in range(numModes):
        ...
        ax.plot(
            lags, meanEigs_markov[:, i + 1], color=cs[j], linestyle="--", linewidth=1
        )
    ...
    ax.legend(
        [
            ax.plot([], [], "-", color=cs[i], label="i = %i" % i)[0]
            for i in range(numModes)
        ],
    ...
    for i in range(numModes):
        ...
        ax.fill_between(
            lags,
            meanEigs[:, i + 1] - meanEigs_markov[:, i + 1] - z,
            meanEigs[:, i + 1] - meanEigs_markov[:, i + 1] + z,
            color=cs[j],
            edgecolor=None,
            alpha=0.4,
    ...
    ax.set_xlabel("# of Transitions", fontsize=16, fontweight="bold")
    ax.set_ylabel(
        r"|$\lambda_i$| - |$\lambda^{Markov}_i$|", fontsize=16, fontweight="bold"
    )
    ax.legend(
        [
            ax.plot([], [], "-", color=cs[i], label="i = %i" % i)[0]
...
def makeregionvideos_mice(
    parameters, h5s, clips, dsetnames, minLength=10, maxlength=90, subs=2
):
    animfps = 30.0
    pad = 100
    ...
    wshedfile = hdf5storage.loadmat(
        parameters.projectPath + "/%s/zVals_wShed_groups.mat" % parameters.method
    )
    groups = wshedfile["groups"] - 1

    indcheck = [
        np.argwhere(np.array(dsetnames) == wzname[0][0].split("_pcaModes")[0])[0][0]
        for wzname in wshedfile["zValNames"][0]
    ]
    ...
    outputdir = parameters.projectPath + "/%s/RegionVids%i/" % (
        parameters.method,
        wshedregs,
    )
    try:
    ...
    for region in range(wshedregs):
        ...
        dnames = [
            dsetnames[0][v].split("/")[-1].split(".")[0].split("session")[0][:-1]
            for v in vidindslist
        ]
        ...
        for i, v in tqdm(enumerate(vidindslist)):
            ...
            for j, f in enumerate(framestoplot[i]):
                ...
                if (
                    xmin < 0
                    or ymin < 0
                    or xmax > frame_region.shape[1] - 1
                    or ymax > frame_region.shape[0] - 1
                ):
                    if (
                        xmax > frame_region.shape[1] - 1 + pad
                        or ymax > frame_region.shape[0] - 1 + pad
                    ):
                        excess = max(
                            frame_region.shape[1] - 1 + pad - xmax,
                            frame_region.shape[0] - 1 + pad - ymax,
                        )
                        frame_region = np.pad(
                            frame_region,
                            (
                                (pad + excess, pad + excess),
                                (pad + excess, pad + excess),
                            ),
                            "minimum",
                        )
                        ...
                        frame_region = np.pad(
                            frame_region, ((pad, pad), (pad, pad)), "minimum"
                        )
                    frame_region = frame_region[
                        ymin + pad : ymax + pad, xmin + pad : xmax + pad
                    ]
        ...
        def make_frame(t):
            ...
            for i in range(subx * subx):
                ...
                xmin = (
                    np.round(h5s[vidindslist[i]][framestoplot[i][j], 8, 0]).astype(
                        "int"
                    )
                    - pad
                )
                ymin = (
                    np.round(h5s[vidindslist[i]][framestoplot[i][j], 8, 1]).astype(
                        "int"
                    )
                    - pad
                )
                ...
                for conn in connections:
                    ax.plot(
                        h5s[vidindslist[i]][framestoplot[i][j], conn, 0] - (xmin),
        ...
        animation.write_videofile(outfile, fps=animfps, audio=False, threads=1)
        print("Video saved at %s." % outfile)
        plt.close()


def makeregionvideo_mice(
    region, parameters, h5s, clips, dsetnames, minLength=10, maxLength=90, subs=2
):
    wshedfile = hdf5storage.loadmat(
        parameters.projectPath + "/%s/zVals_wShed_groups.mat" % parameters.method
    )
        ...
        for j, f in enumerate(framestoplot[i]):
            ...
            if (
                ...
                if (
                    ...
                    frame_region = np.pad(
                        frame_region,
                        ((pad + excess, pad + excess), (pad + excess, pad + excess)),
                        "minimum",
                    )
                else:
                    frame_region = np.pad(
    ...
    def make_frame(t):
        ...
        for i in range(subx * subx):
            ...
            j = j_ % len(framestoplot[i])
            xmin = (
                np.round(h5s[vidindslist[i]][framestoplot[i][j], 8, 0]).astype("int")
                - pad
            )
            ymin = (
                np.round(h5s[vidindslist[i]][framestoplot[i][j], 8, 1]).astype("int")
                - pad
            )
            ax.imshow(frames[i][j], cmap="Greys_r")
            for conn in connections:
...
def makeregionvideo_flies(
    region, parameters, wshedfile, clips, subs=2, minLength=10, maxLength=100
):
    animfps = 50.0
    submaxframes = 500

    groups = makeGroupsAndSegments(
        wshedfile["watershedRegions"][0],
        wshedfile["zValLens"][0],
